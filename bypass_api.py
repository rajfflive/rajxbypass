...
# ========== CONFIG (Change these on Render via Environment Variables) ==========
API_ID = int(os.environ.get('API_ID', 123456))
API_HASH = os.environ.get('API_HASH', 'your_api_hash')
BOT_USERNAME = '@link_bypass_kd_bot'
ADMIN_SECRET = os.environ.get('ADMIN_SECRET', 'SuperSecret@123')
DEFAULT_CREDITS = 0   # 0 = no free credits, sell only

app = Flask(__name__)
loop = asyncio.new_event_loop()
asyncio.set_event_loop(loop)

client = TelegramClient('bypass_session', API_ID, API_HASH, loop=loop)
...
# HTML UI (same as before, shortened)

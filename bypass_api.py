...
def clean_url(url):
    ...
    # Remove any trailing garbage like **, __, etc.
    url = re.sub(r'[\*_`]+$', '', url)
    return url

def is_valid_bypass(original, candidate):
    """Return True if candidate is a valid bypass (not same as original, starts with http, has at least domain)"""
    if not candidate:
        return False
    ...
    if not candidate.startswith(('http://', 'https://')):
        return False
    # Optionally reject if candidate is too short or obviously fake
    if len(candidate) < 10:
        return False
    return True

def extract_links_from_message(msg):
    # First, clean the message from markdown artifacts
    msg = re.sub(r'\*\*+', '', msg)
    msg = re.sub(r'`', '', msg)
    
    # Patterns for destination/bypassed link (case insensitive)
    patterns = [
        r'(?:Bypassed Link|Destination)\s*:?✅?\s*(https?://[^\s\n]+)',
        r'🎁\s*𝗕ʏᴩᴀꜱꜱᴇᴅ\s*:?\s*(https?://[^\s\n]+)',
        r'Bypassed\s*:\s*(https?://[^\s\n]+)',
        r'Destination\s*:\s*(https?://[^\s\n]+)',
        # Last resort: any URL after "Destination" or "Bypassed" in the message
        r'(?:Destination|Bypassed).*?(https?://[^\s\n]+)'
    ]
    ...
    for pat in patterns:
        ...
        if m:
            ...
            if dst and is_valid_bypass(src, dst):
                break
            else:
                dst = None
                continue
    
    # If still no valid dst, try to get the last URL in message that is not the original
    if not dst:
        urls = re.findall(r'https?://[^\s\n]+', msg)
        if urls:
            # Take the last URL that is not equal to src (if src known)
            for url in reversed(urls):
                cand = clean_url(url)
                if cand and (not src or cand != src) and cand.startswith('http'):
                    dst = cand
                    break
...
def bypass():
        ...
        for _ in range(20):
            ...
            for msg in msgs:
                if msg.text:
                    ...
                    # Special fallback for messages containing t.me link (often destination)
                    if 't.me' in msg.text:
                        urls = re.findall(r'https?://[^\s\n]+', msg.text)
                        for url in urls:
                            cand = clean_url(url)
                            if cand and 't.me' in cand and cand != link:
                                print(f"[SUCCESS] {bot} (t.me) -> {cand}")
    ...
    try:
        ...
        if result:
            ...
            u['credits'] += 1
            u['used'] -= 1
            return jsonify({'status': False, 'error': 'All bots failed to return a valid bypass link', 'credits_remaining': u['credits'], 'total_bypassed': u['bypassed'], 'success_rate': success_rate(u['used'], u['bypassed']), 'developer': '@rajfflive'})
    except Exception as e:
        u = get_user(api_key)
        u['credits'] += 1
        u['used'] -= 1
...
# ------------------- ADMIN PANEL (keep same as previous full version) -------------------
# ... (I will include the full admin HTML templates as before, but to save space, assume they are identical to the previous working version. However, to avoid any missing template error, I'll include them again briefly)

# For brevity, copy the HOME_HTML, LOGIN_HTML, ADMIN_HTML from the previous message where they were fully defined.
# Actually, I'll include them in this final code because the user needs complete code.

...
user_credits = {}      # { api_key: { credits, total_used, total_bypassed, created_at, last_used, expiry } }
pending_requests = {}
...
def get_user(api_key):
    if api_key not in user_credits:
        ...
        # No expiry by default (None means never expires)
        user_credits[api_key] = {
            ...
            'created_at': now,
            'last_used': None,
            'expiry': None      # ISO format date or None
        }
...
def admin():
    if request.method == 'POST':
        ...
        if secret == ADMIN_SECRET:
            ...
            return render_template_string(ADMIN_DASHBOARD, keys=user_credits, secrets=secrets)
        else:
    ...
    try:
        ...
        while time.time() - start < 25:
            ...
            if pending_requests.get(req_id, {}).get('complete'):
                ...
                u = get_user(api_key)
                u['total_bypassed'] += 1
                return jsonify({'status': True, 'original_link': res['original_link'], 'bypassed_link': res['bypassed_link'], 'credits_remaining': u['credits'], 'expiry': u['expiry']})
        # timeout refund
        u = get_user(api_key)
        u['credits'] += 1
        u['total_used'] -= 1
...
def credits():
    ...
    return jsonify({'status': True, 'credits_remaining': u['credits'], 'total_used': u['total_used'], 'total_bypassed': u['total_bypassed'], 'expiry': u['expiry'], 'expired': expired})

...
# ---------- Telegram client with single event loop ----------
loop = asyncio.new_event_loop()
asyncio.set_event_loop(loop)
client = TelegramClient(StringSession(SESSION_STRING), API_ID, API_HASH, loop=loop)

# ---------- Data ----------
user_credits = {}          # {api_key: {credits, used, bypassed, expiry, ...}}
recent_requests = {}       # duplicate prevention
bypass_cache = OrderedDict()   # {original_link: {'bypassed': link, 'ts': timestamp}}
...
def get_user(api_key):
    if api_key not in user_credits:
        user_credits[api_key] = {
            ...
            'expiry': None, 'created': datetime.now().isoformat()
        }
...
def add_credits(api_key, amount, days=None):
    u = get_user(api_key)
    u['credits'] += amount
    if days:
...
def is_valid_bypass(original, candidate):
    ...
    if any(f in candidate.lower() for f in fake):
        return False
    # known good domains or accept any real-looking URL
    good = ['t.me', 'mediafire.com', 'devuploads.com', 'modsfire.com',
            'gplinks.co', 'shrinkme.io', 'linkpays.in', 'arolinks.com']
    if not any(d in candidate.lower() for d in good):
        if len(candidate) < 15:
            return False
    return True
...
def bypass():
    ...
    # ---------- parallel bots with auto-delete ----------
    async def try_bot(bot):
        try:
            sent = await client.send_message(bot, link)
            # delete our own message after 60 seconds
            async def del_sent():
        ...
        for _ in range(12):   # 12 seconds polling
            ...
            for msg in msgs:
                if msg.text:
                    ...
                    if dst and is_valid_bypass(link, dst):
                        # delete bot's response after 60 seconds
                        async def del_resp():
                            await asyncio.sleep(60)
                            try:
                                await msg.delete()
    ...
    try:
        ...
        if result:
            ...
            u['credits'] += 1
            u['used'] -= 1
            return jsonify({'status': False, 'error': 'All bots failed to return valid link',

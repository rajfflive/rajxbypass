...
def extract_links_from_message(msg):
    ...
    for pat in src_patterns:
        ...
        if m: src = m.group(1); break
    for pat in dst_patterns:
        m = re.search(pat, msg, re.I)
        if m: dst = m.group(1); break
    if not dst:
        urls = re.findall(r'https?://[^\s\n]+', msg)
        if urls: dst = urls[-1]
    if not src and 'urls' in locals() and urls: src = urls[0]
    return src, dst
...
def admin():
    ...
    if session.get('admin_logged_in'):
        ...
        total_bypassed = sum(x['bypassed'] for x in user_credits.values())
        total_used = sum(x['used'] for x in user_credits.values())
...
def admin_generate():
    ...
    if key_type == 'auto':
        ...
        if not api_key or api_key in user_credits: return jsonify({'status': False, 'error': 'Invalid/duplicate'})

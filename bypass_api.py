...
def extract_links_from_message(msg):
    ...
    if not dst:
        ...
        for url in reversed(urls):
            cand = clean_url(url)
            if cand and (not src or cand != src):
                dst = cand; break
    return src, dst
